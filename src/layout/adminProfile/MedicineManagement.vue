        ...
        <el-dialog v-model="isDialogVisible" :title="dialogTitle">
          ...
          >
            ...
            <el-form-item label="分类" prop="type">
              <el-input v-model="currentMedicine.type" :disabled="isReadOnly" />
            </el-form-item>
...
const getMedicineList = async (pager = 1) => {
  ...
  if (result.code === 200) {
    ...
    // medicines.value = result.data.medicineList;
...
const getMedicineById = async (Id: number) => {
  ...
  if (result.code === 200) {
    ...
    currentMedicine.img = result.data.medicine.img_path;
  ...
  try {
    ...
    if (valid) {
      const updatedMedicine = ref<any>({
        ...
        imgPath: currentMedicine.img || null,
        medicinePrice: currentMedicine.price || null,
        medicineEffect: currentMedicine.effect || null,
      });
      console.log("updatedMedicine", updatedMedicine.value);
      const result: any = await reqAddOrUpdateMedicine(updatedMedicine.value);
      if (result.code === 200) {
        closeDialog();
        getMedicineList();
        ElNotification({
